package com.goodgamestudios.cafe.world.objects.overlay
{
   ...
   public class MovingOverlay extends VisualElement
   {
      ...
      private function get movingOverlayVO() : MovingOverlayVO
      {
         return vo as MovingOverlayVO;
      }
      
      override public function isVisibleIn(param1:Rectangle) : Boolean
      {
         var _loc2_:Boolean = super.isVisibleIn(param1);
         if(!_loc2_)
         {
            die();
         }
      ...
      {
         var _loc1_:Class = getDefinitionByName("Moving_Overlay") as Class;
         ...
         if(this.textField)
         {
            this.textField.text = this.movingOverlayVO.text;
         }
      ...
      {
         dyingTimeFrame = this.movingOverlayVO.lifeTime;
         die(param1);
         this.updateDyingVisRep(param1);
      }
      
      override protected function updateDyingVisRep(param1:Number) : void
      {
         var _loc2_:Number = 1 - (deathTime - param1) / dyingTimeFrame;
         if(_loc2_ < 0)
         {
            return;
         }
         this.overlay.y = Math.sin(_loc2_ * (Math.PI / 2)) * this.movingOverlayVO.relTargetY;
      }
   }
}
